...
- When changing the WeChat public-account RSS report flow, keep the operator-facing rehearsal path (`report-status` -> `daily-report --output` -> `--publish` -> `publish-history`) consistent across examples and docs.
