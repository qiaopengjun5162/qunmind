...
{{
  ...
  "focus_text": "今日最重要的一条新闻，用2-3句说清楚具体事件：谁做了什么、核心亮点是什么、读者可以怎么使用这条信息或应该核对哪些细节。不加个人观点，不写空话",
  "focus_url": "对应URL",
  "ai_items": [
    {{
...
- 每个 focus、section item、read 的文字都必须对应自己的 url 字段；不要写无法追溯到 URL 的观点、判断或归纳。focus_text 不能只写一句标题解释，必须给出可执行核对点；section item 的 comment 要服务于“值得关注”卡片，不要只复述标题。
...
- 日报要对读者有用：每个板块优先回答“读者今天能拿它做什么”，例如核对一个工具、关注一个协议变化、阅读一份方法论、排查一个安全风险、收藏一个官方资料或跟踪一个项目路线。
