...
- news_list 里的「摘要」由各发布方提供，可能带营销套话或免责声明（如"值得关注""建议核对""建议打开原文"）；转述时只取可核验事实，禁止照搬这些句式，comment 和 takeaway 必须是你独立写出的判断。
- 若同一事件近期已被广泛报道，优先保留带来新事实、新数据、新进展的条目；避免重复搬运前几日已覆盖过的角度与素材。
