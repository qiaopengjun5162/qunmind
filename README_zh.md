...
- 如果 `report-status` 或最近回执已经出现 `automation_state = "login_required"`，先执行 `report-login` 复用一次新的公众号后台登录态，再执行 `report-configure` 重试浏览器自动化步骤
...
- **如果回执里看到 `automation_state = "login_required"`**：优先执行 `qunmind report-login --report-name "微信公众号日报"` 或 `just report-login config.toml '微信公众号日报'`，然后执行 `qunmind report-configure --report-name "微信公众号日报"` 或 `just report-configure config.toml '微信公众号日报'`，再重试自动化配置，而不是把问题误判成“系统没有走到自动化”
