...
let bg = CGGradient(colorsSpace: CGColorSpaceCreateDeviceRGB(), colors: [
    color(5, 17, 15).cgColor,
    color(8, 25, 30).cgColor,
    color(17, 20, 19).cgColor
] as CFArray, locations: [0, 0.55, 1])!
ctx.drawLinearGradient(bg, start: point(0, 0), end: point(CGFloat(width), CGFloat(height)), options: [])

ctx.setLineWidth(1)
ctx.setStrokeColor(color(198, 234, 220, 0.055).cgColor)
...
ctx.setFillColor(color(11, 46, 38, 0.34).cgColor)
...
ctx.setStrokeColor(color(107, 232, 184, 0.18).cgColor)
ctx.setLineWidth(2)
...
func line(_ a: CGPoint, _ b: CGPoint, alpha: CGFloat) {
    ctx.setStrokeColor(color(99, 243, 186, alpha).cgColor)
...
for point in [CGPoint(x: 706, y: 156), CGPoint(x: 878, y: 130), CGPoint(x: 1034, y: 108), CGPoint(x: 674, y: 298), CGPoint(x: 896, y: 262), CGPoint(x: 1080, y: 230), CGPoint(x: 738, y: 428), CGPoint(x: 948, y: 392), CGPoint(x: 1136, y: 362)] {
    ctx.setFillColor(color(99, 243, 186, 0.72).cgColor)
    ctx.fillEllipse(in: ellipse(point.x, point.y, 4))
}

for radius in stride(from: CGFloat(104), through: CGFloat(56), by: -16) {
    ctx.setFillColor(color(142, 214, 183, 0.045).cgColor)
    ctx.fillEllipse(in: ellipse(936, 214, radius))
}
ctx.setFillColor(color(235, 247, 234, 0.92).cgColor)
ctx.fillEllipse(in: rect(876, 154, 128, 128))
ctx.setFillColor(color(8, 25, 30, 0.97).cgColor)
ctx.fillEllipse(in: rect(916, 138, 132, 132))

let card = roundedRect(72, 70, 1056, 535, 28)
color(7, 15, 14, 0.34).setFill()
card.fill()
color(199, 234, 220, 0.1).setStroke()
card.lineWidth = 1
card.stroke()

drawText("XUNYUE NOTES", x: 128, y: 134, size: 22, weight: .medium, color: color(214, 184, 116), name: "Menlo", tracking: 4)
drawText("AI · Web3", x: 128, y: 210, size: 74, weight: .bold, color: color(241, 246, 239))
drawText("最新日报", x: 128, y: 294, size: 74, weight: .bold, color: color(241, 246, 239))

ctx.setFillColor(color(99, 243, 186, 0.78).cgColor)
ctx.fill(rect(130, 356, 360, 2))

drawText("每日追踪 AI、Web3 与开源技术信号", x: 130, y: 402, size: 30, weight: .medium, color: color(196, 217, 207))
drawText("DAILY BRIEF · AI / WEB3 / OPEN SOURCE", x: 130, y: 460, size: 21, weight: .regular, color: color(127, 166, 148), name: "Menlo", tracking: 2)

let author = roundedRect(130, 512, 224, 50, 25)
color(15, 36, 31, 0.9).setFill()
author.fill()
color(99, 243, 186, 0.42).setStroke()
author.lineWidth = 1.3
author.stroke()
drawText("寻月隐君", x: 162, y: 546, size: 24, weight: .medium, color: color(236, 247, 240))

let index = roundedRect(800, 418, 248, 102, 18)
color(6, 16, 15, 0.68).setFill()
index.fill()
color(214, 184, 116, 0.32).setStroke()
index.lineWidth = 1.1
index.stroke()
drawText("SIGNAL INDEX", x: 826, y: 458, size: 18, weight: .regular, color: color(214, 184, 116), name: "Menlo", tracking: 2)
drawText("AI  WEB3  AGENT  ZK", x: 826, y: 492, size: 17, weight: .regular, color: color(154, 217, 193), name: "Menlo")

ctx.setStrokeColor(color(232, 246, 238, 0.06).cgColor)
ctx.setLineWidth(2)
