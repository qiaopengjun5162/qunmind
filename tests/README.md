...
Those fixtures now also back the rules that offline `handle-once` replay must target exactly one explicit message, surface candidate message IDs on ambiguous captures, reject direct-chat samples, and reject selected messages that would not reply under the current mention settings.
