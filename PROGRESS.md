- 2026-09-05: **新增 XAirouter AI 热点聚合源** — 用户要求把 `news.xairouter.com` 也作为日报公开来源。站点提供 Atom feed（`/feed.xml`），聚合 Anthropic / OpenAI / Google / NYT / Guardian 等多家媒体与官方博客的 AI 动态。新增 `src/source/xairouter.rs`（3 个单测 + 1 个源级 #[ignore] live fetch 实测）：解析 Atom `<entry>`，归一化 `source="XAirouter"`、score=120、取首个 `<category>` 为分类；注册到 `src/source/registry.rs`，`config.rs` 加 `xairouter_*` 字段（默认关闭），`config.toml` 显式开启 `xairouter_enabled=true`。关键：条目指向外部媒体、URL 不在 `is_curated_source_url` 白名单、标题未必命中 `topic_keywords`（如「Anthropic 版权和解」不含 "ai" 子串），已在 `src/source/mod.rs` 的 `matches_topics` 中按 `source == "XAirouter"` 始终纳入素材池，并在 `is_curated_source_url` 补挂 `xairouter.com` 双保险，避免被关键词过滤误丢。验证：`cargo check --lib` 通过、`cargo test --lib source::xairouter` 3 passed、源级 live fetch 实测拉到 12 条并正确归一化、`cargo build --release` 通过（`target/release/qunmind` 已含新源）。约束已写入 AGENTS.md 与中英文 README。改动随 `feat/xairouter-source` 提 PR。

- 2026-08-17: **AI 优先日报 prompt 收口（收口 PR #50 已知 warn 项）** — 在 `src/daily_report/prompt.rs` 补两条约束：(1) 转述时禁止照搬 news_list「摘要」字段里发布方自带的营销套话/免责声明（如"值得关注""建议核对""建议打开原文"），comment/takeaway 必须是模型独立判断；(2) 同一事件近期已广泛报道时优先保留带新事实/新数据/新进展的条目、避免重复搬运前几日角度。lint 侧已有 `LOW_CONFIDENCE_PHRASES`（20 条）检测焦点/正文/深读区套话、`lint_recent_overlap` 与 `mod.rs` 的 `drop_recent_duplicates_*` 处理跨日重合；本次在 prompt 软约束层收口，使模型更少触发这些 warn。验证：cargo clippy --lib 零警告、cargo test --lib 487 passed / 3 ignored。改动随 `feat/ai-first-daily-report` 推到 PR #50。

- 2026-08-16: **AI 生成日报已推送并发送手机预览（改版后真正落地的第一版）** — `moonpub push /tmp/wechat-report-2026-08-16-ai-final2.md --render` 完整跑通（Session 恢复 → 原创/赞赏/留言/创作来源落库 → 草稿保存 → 预览发送手机 qpj4812701762 ret=0），media_id `EmukC2rjB9X3nj6feGSEr64tu4D_zksnvaChzmde9p-gemcwYJRzJkPUZv84BblL`。正文是 AIIA 风格的真实 AI 内容（如"daos.fun 创始人指控 ai16z 核心成员内幕抛售获利 660 万美元：为什么值得关注…链上取证或成行业刚需"）。残余已知问题：AI 偶发在焦点/HN 英文条目输出套话（"Web3相关主题，建议打开原文核对…"）与 focus_text/focus_url 不匹配，可后续加强 prompt 约束；lint 无 error 不阻断发布。草稿待人工点「发表」。

- 2026-08-16: **日报真正启用 AI 生成（根因修复）** — 用户反馈"内容都不全、没变化"，排查发现 2026-07-11 起公众号日报实际一直走 generate_deterministic*（Rust 确定性配方），从未调用 AI，此前对 prompt/types/render 的 AIIA 改版对实际产出零影响，手机上看到的全是 fallback 套话。修复：新增 generate_with_ai_fallback / generate_from_curated_items_with_fallback（AI 优先、失败回退确定性配方），并把 reporting.rs 的 generate_manual_public_daily_report / generate_group_report_from_store 与 scheduler/daily_report.rs 三处生产路径全部切到 AI 优先；同步更新 AGENTS.md 决策（原"必须走 deterministic"改为"AI 优先+deterministic 兜底"）。真实生成验证：AI 路径产出 AIIA 风格内容（"亚利桑那州助 Web3 ATM 受害者追回全款：发生了什么…已帮 35 名受害者追回 17.13 万美元；为什么值得关注…可能推动其他州出台类似规则"），lint 警告从 6 条降到 3 条。适配测试：scheduler 与 main.rs 中"群消息不调用 AI"的旧断言改为"调用一次 AI"。验证：535 nextest 全过、clippy 零警告。

- 2026-08-16: **AIIA 改版日报已推送草稿并发送手机预览** — 改版后首份真实发布：`moonpub push /tmp/wechat-report-2026-08-16-aiia-final11.md --render` 完整跑通浏览器自动化（Session 恢复 → 原创声明/赞赏/留言/创作来源全部落库 → 草稿保存 → 预览发送到手机 qpj4812701762 ret=0），media_id `EmukC2rjB9X3nj6feGSEr7ifSgmiDE8PIMYOEr6XwrSdDbte_7Es0WSikBCAq315`，并自动删除同日旧草稿。后台 `draft/batchget` 确认优化版已是草稿箱第一条（digest "GMGN：Solana、BNB、Robinhood 三链 Meme 日交易量超 2.5 亿美元"）。个人订阅号仍无法自动发表，草稿待人工点「发表」。

# Progress

- 2026-08-28: **评估并收口 last30days 集成边界** — 核对 `mvanhorn/last30days-skill` 的公开 README、仓库结构和近期提交后，确认值得吸收的是最近 30 天新鲜度、多源并行、互动信号、跨来源事件聚类和结构化 JSON 研究结果；不把 Python runtime、ScrapeCreators、yt-dlp、平台 Cookie 或 MCP backend 并入 QunMind。新增 `docs/last30days-integration.md`，明确 helper -> staging -> 人工审核 -> `manual_items` / research memory 的回流路径，后续优先实现只读 doctor 和 JSON 适配。

- 2026-08-16: **日报改版：参考 AIIA 周刊引入事实性短评** — 对比《AIIA周刊丨人工智能行业一周精选》后发现当前日报"没人看"的主因是内容只有转述、没有编辑判断，且充满"值得关注：…建议核对…"套话与截断英文标题。本轮改版：(1) prompt 升级为"资深编辑"角色，要求每条输出精炼中文标题（15-25 字、禁止截断英文）+ 具体事实 comment（谁做了什么、含数字）+ 一句事实性短评 takeaway（为什么值得关注/影响谁/下一步看什么，限事实判断不预测行情），并给出完整 few-shot 示例；(2) types 新增 `takeaway` / `focus_takeaway` 字段；(3) render 改为 AIIA 式排版：每条 = `发生了什么 → 为什么值得关注 → 来源依据 → 原文入口`，焦点区同步；(4) 新增 enrich 后处理 `localize_ellipsis_titles`，把模型输出仍带省略号的英文标题/comment/takeaway 用新闻项完整标题与基于来源的短评兜底；(5) 截断统一改中文省略号"…"避免 ASCII "..." 误判；(6) lint 词表扩充到 20 条套话检测、检查 marker 改为新字段。验证：535 个 nextest 全过、clippy 零警告、fmt clean；真实生成 `/tmp/wechat-report-2026-08-16-aiia-final11.md`，lint 无 error，套话警告从 6 条降到 3 条，首屏焦点不再有"建议核对原文"后缀。仍保留 warn 项：个别源摘要自带套话（已尽量剥离）、跨日素材重合提醒。注：`config.example.toml` 的 taplo 格式问题为上游分支已存在，非本轮引入。
