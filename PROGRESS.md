...
- `cargo nextest run --all-features wx_cli_formal_test_plan`：13 targeted formal wx-cli test-plan tests passing.
...
- `cargo nextest run --all-features`：150 tests passing, 1 ignored PostgreSQL integration test compiled and skipped by default.
- `cargo llvm-cov nextest --all-features --summary-only`：89.05% line coverage, 150 tests passing.
