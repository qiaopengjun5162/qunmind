...
如果最近回执或 `report-status` 已给出 `automation_state = "login_required"`，下一步优先使用项目内标准入口 `qunmind report-login` 或 `just report-login` 复用 `moonpub` 浏览器登录态，不要只把动作描述成手工跑裸 `moonpub login`。日报目标选择、`wechat_bin` 和 `wechat_articles_dir` 解析应继续走 QunMind 自己的配置边界。
