...
`[[schedule.daily_reports]]` 中 `output = "wechat"` 的目标依赖本地 `moonpub`，当前调用形态是 `moonpub --articles <dir> push <temp_markdown_file> --render`。这类目标至少要检查三件事：`wechat_bin` 可调用、`wechat_articles_dir` 指向可用的 moonpub articles 根目录、以及 `[public_sources]` 至少启用一个来源；否则它更像“已声明计划”而不是“可执行日报目标”。新增这类诊断时优先放在 `diagnostic` 纯函数报告里前置暴露，不要等到定时任务运行时报错才发现。
