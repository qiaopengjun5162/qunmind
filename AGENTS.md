...
公众号日报排版优先使用 `moonpub` 已支持的 block fence，例如 `intro`、`tip`、`callout`、`divider`、`steps`、`timeline` 和 `summary`。当前正文结构是“导语 -> 今日速览 -> 今日焦点 -> 编号 AI / Web3 / 技术 / 深读分区 -> 参考来源 / 完整素材链接”；后续优化排版时先确认 `moonpub` renderer 支持对应 block，不要随手造新语法导致草稿渲染退化。
