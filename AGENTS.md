...
公众号日报封面当前继续交给 `moonpub --render` 默认生成。`QunMind` 侧优先把精力放在标题、导语、板块归类和引用链接质量上，不在 CLI、MCP、publisher 里额外注入本地 `cover:` 资产，避免内容链路和平台渲染链路再次分叉。
为了让公众号草稿箱里的“最新一条”更容易辨认，日报主标题当前优先固定为 `微信公众号日报｜YYYY-MM-DD`。当天真正变化的主线信息应继续放在 `digest`、`intro` 和 `今日焦点`，不要再把公共素材主线直接塞回 `title`，否则草稿箱里会重新出现“像同一篇又像不同篇”的识别噪音。
